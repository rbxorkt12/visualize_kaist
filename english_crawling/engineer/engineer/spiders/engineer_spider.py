import scrapy
from engineer.items import EngineerItem

class EngineerSpider(scrapy.Spider):
    name = "engineer"

    def start_requests(self):
        base_url = 'https://www.theengineer.co.uk/page/'
    ...
    def parse(self, response):
        url_list = response.xpath('//*[@class="search-results"]/article/h2/a/@href').extract()
        for url in url_list:
            article_url = url
    ...
    def parse_article(self, response):
        ...
        article_title = response.xpath('//*[@class="page-title large"]/text()').extract()[0]
        article_title = article_title.strip()
        article_data = ''.join(response.xpath('//*[@class="entry-content"]/p/text()').extract())
